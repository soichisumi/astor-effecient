...
public abstract class AstorCoreEngine {
    ...
    public void showResults2() {
        ...
        log.debug("\nAll variants:");
        //pw.write("variant details:\n");
        //pw.write("id,");
        for (ProgramVariant variant : variants) {
            //pw.write(variant.getId() + "," + variant.getOperations().get(0)); //iminai
            log.debug("f " + variant.getFitness() + ", " + variant);
        }
        ...
        if (!solutions.isEmpty()) {
            log.info("\nSolution details");
            log.info(getSolutionData(solutions, generationsExecuted));
            pw.write("Solution details:\n");
            pw.write(getSolutionData(solutions, generationsExecuted));

        }
    }
    //write a part of solution data
    public void showResults3(PrintWriter pyo) {
        log.info("\n----SUMMARY_EXECUTION---");
        ...
        log.debug("\nAll variants:");
        //pyo.write("variant details:\n");
        //pyo.write("id,");
        for (ProgramVariant variant : variants) {
            //pyo.write(variant.getId() + "," + variant.getOperations().get(0));
            log.debug("f " + variant.getFitness() + ", " + variant);
        }
    ...
    private void outputVariant(ProgramVariant newVariant) {
        pw.write(LineSeparator.Unix + String.valueOf(newVariant.getId()) + ",");
        if (newVariant.getOperations().values().size() > 0) {
            for (List<ModificationInstance> mi : newVariant.getOperations().values()) {
                pw.write("recommended or not:" + (mi.get(0).isRecommendedStmt ? "recommended" : "random search"));  pw.write(LineSeparator.Unix);
                pw.write(mi.get(0).getOperationApplied().toString() + ":\n" + mi.get(0).getOriginal() + "to\n" + mi.get(0).getModified());
                if (mi.size() > 1) pw.write("+alpha");
                pw.write(", ");
            }
            pw.write(LineSeparator.Unix);
        } else {
            pw.write("no operation. what happened" + LineSeparator.Unix);
        }
    }
    ...
    public String getSolutionData2(List<ProgramVariant> variants, int generation) {
        String line = "";
        for (ProgramVariant solutionVariant : variants) {
            line += "solution variant ID:" + solutionVariant.getId() + "\ntime:" + TimeUtil.getDateDiff(this.dateInitEvolution, solutionVariant.getBornDate(), TimeUnit.SECONDS) + "sec"; //id,time
            ...
            for (int i = 1; i <= generation; i++) {
                ...
                for (ModificationInstance genOperationInstance : genOperationInstances) {
                    ...
                    line += LineSeparator.Unix + "operation:" + genOperationInstance.getOperationApplied().toString() + "," + genOperationInstance.getModificationPoint().getCtClass().getQualifiedName() + ",";    //operation,location
                    line += "\noriginal:";
                    line += genOperationInstance.getOriginal().toString() + ",";
                    line += "\nmodified:";
                    if (genOperationInstance.getModified() != null)
